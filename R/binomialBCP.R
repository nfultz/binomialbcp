...
binomialbcp <- function(x, n , prior=list(a=.5, b=.5), control=bbbcp.control()) {
...
#'
#' This function only exists to make autocomplete work.
#'
#' @export
#'
...
bbbcp.control <- function(mcmc.iterations=10, mcmc.burnin=500, mcmc.thin=20, verbose=FALSE) {
  as.list(environment())
}
